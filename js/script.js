...
function calculatePythagoras(dateStr) {
    ...
    // 3. S2 — сумма цифр S1
    const S2 = S1.toString().split('').reduce((sum, n) => sum + Number(n), 0);
    ...
    const S3 = S1 - 2 * firstSignificantDigit;

    // 6. S4 — сумма цифр S3 (берём модуль, если отрицательное)
    const S4 = Math.abs(S3).toString().split('').reduce((sum, n) => sum + Number(n), 0);

    // 7. собираем ВСЕ цифры в правильном порядке
    const allDigits = [
        ...dateDigits,           // цифры даты: 2,8,0,2,1,9,8,6
        ...S1.toString().split('').map(Number),  // цифры S1: 3,6
        S2,                      // S2: 9
        ...Math.abs(S3).toString().split('').map(Number),  // цифры S3: 3,2
        S4                       // S4: 5
    ];
    ...
    return {
        ...
        S3,
        S4,
        firstSignificantDigit, // для отладки
        matrix
    };
}
